...
import { updateAssignment, updateUser } from "../graphql/mutations";
...
export default function UserUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = userRecord
      ? { ...initialValues, ...userRecord, assignments: linkedAssignments }
      : initialValues;
  ...
  const [linkedAssignments, setLinkedAssignments] = React.useState([]);
  const canUnlinkAssignments = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : userModelProp;
      const linkedAssignments = record?.assignments?.items ?? [];
      setLinkedAssignments(linkedAssignments);
      setUserRecord(record);
    };
    queryData();
  }, [idProp, userModelProp]);
  React.useEffect(resetStateValues, [userRecord, linkedAssignments]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const assignmentsToUnLink = [];
          const assignmentsSet = new Set();
          const linkedAssignmentsSet = new Set();
          assignments.forEach((r) =>
            assignmentsSet.add(getIDValue.assignments?.(r))
          );
          linkedAssignments.forEach((r) =>
            linkedAssignmentsSet.add(getIDValue.assignments?.(r))
          );
          linkedAssignments.forEach((r) => {
            if (!assignmentsSet.has(getIDValue.assignments?.(r))) {
              assignmentsToUnLink.push(r);
            }
          });
          assignments.forEach((r) => {
            if (!linkedAssignmentsSet.has(getIDValue.assignments?.(r))) {
              assignmentsToLink.push(r);
            }
          });
          assignmentsToUnLink.forEach((original) => {
            if (!canUnlinkAssignments) {
              throw Error(
                `Assignment ${original.id} cannot be unlinked from User because owner is a required field.`
              );
            }
            promises.push(
              client.graphql({
          ...
          assignmentsToLink.forEach((original) => {
            promises.push(
              client.graphql({
                query: updateAssignment.replaceAll("__typename", ""),
                variables: {
          ...
          const modelFieldsToSave = {
            ...
            contactNumber: modelFields.contactNumber ?? null,
          };
          promises.push(
            client.graphql({
