...
import { getAssignment, getUser, listUsers } from "../graphql/queries";
...
export default function AssignmentUpdateForm(props) {
  ...
  const [selectedOwnerRecords, setSelectedOwnerRecords] = React.useState([]);
  const autocompleteLength = 10;
  const [errors, setErrors] = React.useState({});
  const resetStateValues = () => {
    const cleanValues = assignmentRecord
      ? { ...initialValues, ...assignmentRecord, owner }
      : initialValues;
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : assignmentModelProp;
      const ownerRecord = record ? record.owner : undefined;
      const userRecord = ownerRecord
        ? (
            await client.graphql({
              query: getUser.replaceAll("__typename", ""),
              variables: { id: ownerRecord },
            })
          )?.data?.getUser
        : undefined;
      setOwner(ownerRecord);
      setSelectedOwnerRecords([userRecord]);
      setAssignmentRecord(record);
    };
    queryData();
  }, [idProp, assignmentModelProp]);
  React.useEffect(resetStateValues, [assignmentRecord, owner]);
  ...
  const fetchOwnerRecords = async (value) => {
    ...
    while (newOptions.length < autocompleteLength && newNext != null) {
      ...
      var loaded = result.filter((item) => owner !== item.id);
    ...
    >
        ...
        getBadgeText={(value) =>
          value
            ? getDisplayValue.owner(
                ownerRecords.find((r) => r.id === value) ??
                  selectedOwnerRecords.find((r) => r.id === value)
              )
            : ""
        }
        setFieldValue={(value) => {
          ...
          setCurrentOwnerValue(value);
          const selectedRecord = ownerRecords.find((r) => r.id === value);
          if (selectedRecord) {
            setSelectedOwnerRecords([selectedRecord]);
          }
        }}
      ...
      >
        <Autocomplete
          ...
          onBlur={() => runValidationTasks("owner", currentOwnerValue)}
          errorMessage={errors.owner?.errorMessage}
          hasError={errors.owner?.hasError}
          ref={ownerRef}
          labelHidden={true}
