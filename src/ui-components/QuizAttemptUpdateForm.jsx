...
import { getQuizAttempt } from "../graphql/queries";
import { updateQuizAttempt } from "../graphql/mutations";
const client = generateClient();
export default function QuizAttemptUpdateForm(props) {
  const {
    id: idProp,
    quizAttempt: quizAttemptModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = quizAttemptRecord
      ? { ...initialValues, ...quizAttemptRecord }
      : initialValues;
    setScore(cleanValues.score);
    setCompleted(cleanValues.completed);
    setUserAnswers(
      typeof cleanValues.userAnswers === "string" ||
        cleanValues.userAnswers === null
        ? cleanValues.userAnswers
        : JSON.stringify(cleanValues.userAnswers)
    );
    setAssignmentID(cleanValues.assignmentID);
    setUserId(cleanValues.userId);
    setOwnerRole(cleanValues.ownerRole);
    setErrors({});
  };
  const [quizAttemptRecord, setQuizAttemptRecord] =
    React.useState(quizAttemptModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getQuizAttempt.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getQuizAttempt
        : quizAttemptModelProp;
      setQuizAttemptRecord(record);
    };
    queryData();
  }, [idProp, quizAttemptModelProp]);
  React.useEffect(resetStateValues, [quizAttemptRecord]);
  const validations = {
    score: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateQuizAttempt.replaceAll("__typename", ""),
            variables: {
              input: {
                id: quizAttemptRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "QuizAttemptUpdateForm")}
      {...rest}
    >
      <TextField
        label="Score"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || quizAttemptModelProp)}
